...
const TemporaryAdminPanel = () => {
  ...
  const fetchPendingPayments = async () => {
    try {
      setLoading(true);
      // This calls the backend admin route
      const response = await API.get("/admin/pending-payments");
      setPendingBookings(response.data);
      setError(null);
    } catch (err) {
      setError("Failed to fetch pending payments. Are you logged in as Admin?");
      setPendingBookings([]);
    } finally {
      setLoading(false);
    }
  ...
  // Also deny access if not admin, even after loading
  if (user?.role !== "admin")
      ...
      <div className="space-y-4">
        {pendingBookings.length > 0 ? (
          pendingBookings.map((booking) => {
            ...
            const userName = booking.userId?.name || "Unknown User"; // User name should come from backend populate if needed
              ...
              <div key={booking._id} className="card bg-base-200 shadow-md">
                <div className="card-body">
                  ...
                  <p>
                    ...
                    {/* We need the full URL to the server */}
...
export default TemporaryAdminPanel;
